...
class ViewAnalyticsTest < Minitest::Test
  ...
  def test_identifies_web_browser_from_user_agent
    create_database(3)
    assert_equal ({"Chrome" => 3}), @source.web_browsers
  end

  def test_identifies_os_from_user_agent
    create_database(2)
    assert_equal ({"OS X 10.8.2"=>2}), @source.os
  end

  def test_shows_screen_resolution
    create_database(2)
    assert_equal ({"1920 X 800"=>1, "1920 X 801"=>1}), @source.resolution
  end
